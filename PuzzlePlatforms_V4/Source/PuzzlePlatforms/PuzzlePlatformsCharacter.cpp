...
void APuzzlePlatformsCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(APuzzlePlatformsCharacter, CurrentMessage);
}

void APuzzlePlatformsCharacter::AttemptToSendChatMessage(const FString& Message)
{
	APlayerController* PlayerController = GetWorld()->GetFirstPlayerController();

	// 서버가 아니라면, ServerSendChatMessage( ) 를 호출! 아닌 경우엔 SendChatMessage( ) 호출!
...
void APuzzlePlatformsCharacter::SendChatMessage(const FString& Message)
{
	CurrentMessage = Message;
	UpdateChatText();

	FTimerHandle DummyHandle;
	GetWorldTimerManager().SetTimer(DummyHandle, this, &APuzzlePlatformsCharacter::ClearChatMessage, 5.f);
}

void APuzzlePlatformsCharacter::ClearChatMessage()
{
	CurrentMessage = "";
	UpdateChatText();
}

void APuzzlePlatformsCharacter::ServerSendChatMessage_Implementation(const FString& Message)
{
	SendChatMessage(Message);
}

bool APuzzlePlatformsCharacter::ServerSendChatMessage_Validate(const FString& Message)
{
	bool bValue;

	bValue = Message.Len() < 255 ? true : false;

	return bValue;
}

void APuzzlePlatformsCharacter::OnRep_CurrentMessage()
{
	UpdateChatText();
}

void APuzzlePlatformsCharacter::UpdateChatText()
{
